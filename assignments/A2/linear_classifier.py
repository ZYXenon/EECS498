...
def svm_loss_vectorized(
    ...
    margins = scores - correct_class_score.unsqueeze(1) + 1
    ...
    #############################################################################
    # Replace "pass" statement with your code
    mask = (margins > 0).to(X.dtype)   # shape: (N, C)

    # 7. For correct classes, subtract per-row sum of positives
    mask[torch.arange(num_train), y] = -mask.sum(dim=1)

    # 8. Compute gradient dW: X.T @ mask
    dW = X.t().mm(mask) / num_train

    # 9. Add regularization gradient
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################

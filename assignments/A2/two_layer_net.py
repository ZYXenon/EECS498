...
def nn_forward_backward(
    ...
    # Replace "pass" statement with your code
    partial = probs.clone()
    partial[torch.arange(N), y] -= 1
    partial /= N

    dh1 = torch.mm(partial, W2.T)
    dh1[h1 <= 0] = 0
    grads["W2"] = torch.mm(h1.T, partial) + 2 * reg * W2
    grads["b2"] = torch.sum(partial, dim=0)
    # grads["W1"] = torch.mm(X.T, torch.maximum(torch.zeros_like(h1), h1)) + 2 * reg * W1
    # grads["b1"] = torch.sum(torch.maximum(torch.zeros_like(h1), h1), dim=0)
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    ...
                    print('lr %e hs %d reg %e lr_decay %e val accuracy: %f' % (
                        lr, hs, reg, lr_decay, val_acc))
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
